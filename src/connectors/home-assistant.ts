  ...
  return {
    ...
    async fetch(): Promise<ConnectorResult> {
      ...
      if (entityIds.length > 0) {
        ...
        for (const state of (await resp.json()) as Record<
          string,
          unknown
        >[]) {
          ...
          if (
            ...
            const attrs = (state.attributes ?? {}) as Record<
              string,
              unknown
            >;
            sensors.push({
...
export function validate(config: ConnectorConfig): Check[] {
  ...
  checks.push(
    url ? [PASS, `URL: ${url}`, ""] : [FAIL, "No URL configured", ""],
  );

  const token = process.env[tokenEnv] ?? "";
  checks.push(
    token
      ? [PASS, `${tokenEnv} is set`, ""]
      : [FAIL, `${tokenEnv} is NOT set`, ""],
  );
